 ...
 * results.json et errors.json dans eval-outputs/<label>/.
 */
...
export async function evaluateExample(opts: {
  ex: Example;
  modes: Mode[];
  retriever: Retriever;
  pageStore: PageStore;
  ...
  const { ex, retriever, pageStore, workflow, judge } = opts;
  const question = ex.question.trim();
  const t0 = Date.now();
  const scope = await withBackoff(() => retriever.route(question), `routage ${ex.id}`);
  const docs = await withBackoff(() => retriever.invokeWithScope(question, scope), `retrieval ${ex.id}`);
  const retrieval = { ...retrievalMetrics(docs, ex, opts.kValues, opts.tolerance), retrieval_sec: (Date.now() - t0) / 1000 };
  ...
  for (const mode of opts.modes) {
      ...
      run = await withBackoff(
        () =>
          traced("financebench-question", { id: ex.id, question, mode }, () =>
            workflow.run({ question, passages: docs, retriever, pageStore, scope, mode }),
    ...
    const flags = pageFlags(llmDocs, ex.gold_pages, opts.tolerance);
    Object.assign(row, {
...
async function main() {
  ...
  await pool(
    ...
    Number(args.workers),
    (ex) =>
      evaluateExample({ ex, modes, retriever, pageStore, workflow, judge, kValues, tolerance }).catch((error) => {
    ...
    (result, ex) => {
      ...
      const verdicts = result.map((r) => (r.failed ? "ÉCHEC" : (r.verdict ?? "ok"))).join("/") || "ÉCHEC";
      log(`[${done}/${dataset.length}] ${ex.id} (${ex.doc_name}) -> ${verdicts}`);
    },
  );
  ...
  const summary: Record<string, any> = {
    ...
    config: modelConfig(),
    settings: { research_top_k: settings.RESEARCH_TOP_K, max_tool_calls: settings.GENERATOR_MAX_TOOL_CALLS, tools: env.GENERATOR_TOOLS_ENABLED },
    documents: docs,
